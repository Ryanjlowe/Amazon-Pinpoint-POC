...
The Sample files [asset_enpoints.json](asset_enpoints.json) and [asset_enpoints.csv](asset_enpoints.csv) contains a sample dataset of end user data in the form of endpoints.  The attributes in the sample dataset are defined in the **Import Endpoint Data** section below but are good references for both personalization and segmentation attributes.  This sample file can be used to run the below guide, but should be also used as a reference for real customer end-user data.  (Note: This can also be in a CSV format)
